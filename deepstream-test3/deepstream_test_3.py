...
def ObjectPosition(pA, pB, pP):
    '''
    pA = Point A
    pB = Point B
    pP = Point P
    To determine which side of the line from A=(x1,y1) to B=(x2,y2) a point P=(x,y) 
    falls on you need to compute the value:-
    d=(x-x1)(y2-y1)-(y-y1)(x2-x1)
    '''
    d = (pP[0]-pA[0])*(pB[1]-pA[1])-(pP[1]-pA[1])*(pB[0]-pA[0])
    #d = (pP[1]-pA[1])*(pB[0]-pA[0])-(pP[0]-pA[0])*(pB[1]-pA[1])
    return d
...
def GetRectBox(obj_meta, with_offset=False, xyxy=True):
    ...
    rect_box=None
    if xyxy:
        rect_box=[top, left, top + height, left + width]
    else:
        rect_box=[top, left, height, width]
    return rect_box


def CentreCoord(box, centre_bottom=False, centre_top_bottom=False, top_bottom_side=False):
    centre_coords = (int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    if centre_bottom:
        centre_coords = (int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])))
    if centre_top_bottom:
        centre_coords_T = (int(box[0]+(box[2]-box[0])/2), int(box[1]))
        centre_coords_B = (int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])))
        centre_coords=(centre_coords_B, centre_coords_T)
    if top_bottom_side:
        centre_coords_T = (int(box[0]+(box[2]-box[0])/2), int(box[1]))
        centre_coords_B = (int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])))
        centre_coords_S = (int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2)) #(int(box[2]), int(box[1]+(box[3]-box[1])/2))
        centre_coords=(centre_coords_B, centre_coords_T, centre_coords_S)
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            try: 
                ...
                break
            bbox=GetRectBox(obj_meta)
            bbox=YX_XY(bbox)
            _coordinates=CentreCoord(bbox,centre_bottom=True)
            #d=ObjectPosition((642,),(1870,235),_coordinates)
            #print(d)
            if obj_meta.class_id == PGIE_CLASS_ID_VEHICLE and _coordinates[1]>235 and obj_meta.confidence > 0.5:
                print("Vehicle Detected")
                v_frame = pyds.get_nvds_buf_surface(hash(gst_buffer), frame_meta.batch_id)
                v_frame = np.array(v_frame, copy=True, order='C')
                v_frame = cv2.cvtColor(v_frame, cv2.COLOR_RGBA2BGRA)
                tempvar=datetime.now().strftime("%Y%m%d%H%M%S%f")
                filename=f"/home/nvidia/vehicle_images/{tempvar}.jpg"
